  ...
  <section class="overview">
    <div class="row justify-content-between gx-0 gx-sm-1">
          ...
          <template #adv="{ handleClose }">
            <slot name="filterAdv" :result-count="itemsCount" :search="updateOverviewRoute"
              ...
              <component :is="FilterAdv" v-model="searchObject" :result-count="itemsCount"
                @filter="updateOverviewRoute(true)" @change="updateOverviewRoute(true)" @close="handleClose" />
            </slot>
          </template>
        </Filter>
      </div>
      ...
      <div class="col-auto order-2 order-lg-3 ps-2">
        <RouterLink v-if="!$isReadonlyUser" :to="{ name: Entity.name + 'Details', params: { id: 'new' } }"
          class="btn btn-info">
    ...
    <LoadingContainer :is-loading="isLoading">
      <component :is="List" :readonly="$isReadonlyUser" v-if="items && items.length > 0" v-model="items"
        @request-save="handleRequestSave" @request-remove="handleRequestRemove" @save="handleSave"
        @remove="handleRemove" @request-reload="updateOverviewRoute(false)" />
